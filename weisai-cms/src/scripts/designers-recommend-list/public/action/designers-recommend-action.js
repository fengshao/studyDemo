...
function DesignersRecAction() {
	...
	this.getDesignersRecList = function (typeID) {
		var _this = this;
	...
	this.addSpecial = function (newData) {
		...
		PublicAjax.addSpecial(newData).then(function (list) {
			//_this.dispatch(list.data);
			_this.actions.getDesignersRecList(newData.type_id);
		}, function (errorMsg) {
			_this.dispatch(errorMsg);
		});
	};
...
module.exports = alt.createActions(DesignersRecAction);