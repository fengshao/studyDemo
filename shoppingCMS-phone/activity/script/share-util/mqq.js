...
(function (mqq, argb) {
		...
		function d(b) {
			...
			if (mqq.debuging && c && c.log && "pbReport" !== b.method) try {
				c.log(b)
			} catch (d) {
			}
		}
		...
		function f(b, c, d, e, f) {
			if (mqq.__needReport !== !1 && b && c && d) {
				var g, h, i, j, k = b + "://" + c + "/" + d;
				if (e = e || [], !f || !M[f] && !window[f])
					for (f = null, h = 0, i = e.length; i > h; h++)
						if (g = e[h], mqq.isObject(g) && (g = g.callbackName || g.callback), g && (M[g] || window[g])) {
							f = g;
							break
						}
				f && (N[f] = {
		...
		function h(b, c, d) {
			var e, f;
			return (b = mqq.isFunction(b) ? b : window[b]) ? (e = i(b), f = "__MQQ_CALLBACK_" + e, window[f] = function () {
		...
		function k(c, f, g, h) {
			var i, k, l, m = mqq.isFunction(c) ? c : M[c] || window[c],
				n = Date.now();
			f = f || [], i = f[0], mqq.isUndefined(h) && (h = !0), mqq.isObject(i) && ("data" in i || (i.data = e({}, i)), "code" in i || (i.code = j(i) || 0), i.msg = i.msg || ""), mqq.isFunction(m) ? h ? setTimeout(function () {
				m.apply(null, f)
				...
				url   : l.uri
			}), Number(c) && delete N["__MQQ_CALLBACK_" + c], i && (i.code !== argb ? k = i.code : /^-?\d+$/.test(String(i)) && (k = i)), B.send(l.uri + "#callback", k, n - l.startTime))
		}

		function l(b) {
			var c = E.call(arguments, 1);
			mqq.android && c && c.length && c.forEach(function (b, d) {
				mqq.isObject(b) && "r" in b && "result" in b && (c[d] = b.result)
			}), k(b, c)
		}
		...
		function n(b, c) {
			var d = null,
				...
				j = f[f.length - 1],
				k = g(b.substring(0, h));
			(!k[j] || mqq.debuging) && ((d = c[mqq.platform]) || "browser" === e || ((d = mqq.iOS && c.iOS) ? e = "iOS" : (d = mqq.android && c.android) && (e = "android")), d && c.supportInvoke && (P[i + "." + j] = d), k[j] = d || m, c.support && c.support[e] && (O[i + "." + j] = c.support[e]))
		}

		function o(b) {
			var c, d, e = b.split("."),
				f = e[e.length - 2] + "." + e[e.length - 1];
			return c = O[f] || O[b.replace("qw.", "mqq.").replace("qa.", "mqq.")], mqq.isObject(c) && (c = c[mqq.iOS ? "iOS" : mqq.android ? "android" : "browser"]), c ? (d = c.split("-"), 1 === d.length ? mqq.compare(d[0]) > -1 : mqq.compare(d[0]) > -1 && mqq.compare(d[1]) < 1) : !1
		}
		...
		function p(c, e, f, g) {
			...
			return j.style.cssText = "display:none;width:0px;height:0px;", mqq.iOS && (j.onload = h, j.src = c), (document.body || document.documentElement).appendChild(j), mqq.android && (j.onload = h, j.src = c), i = mqq.__RETURN_VALUE, mqq.__RETURN_VALUE = argb, setTimeout(function () {
				j.parentNode.removeChild(j)
			}, 0), i
		}

		function q(b, c) {
			if ("AndroidQQ" === mqq.platform) {
				if (mqq.compare("4.7.2") < 0) return !0;
				if (S[b] && mqq.compare(S[b]) < 0) return !0
			}
			return !1
		}
		...
		function r(c, d, e, g) {
			...
			var h, j, l, m;
			if (l = E.call(arguments, 2), g = l.length && l[l.length - 1], mqq.isFunction(g) ? l.pop() : mqq.isUndefined(g) ? l.pop() : g = null, e = l[0], j = i(g), -1 === T.indexOf(d) && f("jsbridge", c, d, l, j), g && mqq.isObject(e) && !e.callback && (window["__MQQ_CALLBACK_AUTO_" + j] = g, e.callback = "__MQQ_CALLBACK_AUTO_" + j), q(c, d))
				if (mqq.compare("4.5") > -1 || /_NZ\b/.test(C)) h = "jsbridge://" + encodeURIComponent(c) + "/" + encodeURIComponent(d) + "/" + j, l.forEach(function (b) {
					mqq.isObject(b) && (b = JSON.stringify(b)), h += "/" + encodeURIComponent(String(b))
				}), p(h, c, d, j);
				else if (window[c] && window[c][d]) {
					if (m = window[c][d].apply(window[c], l), !g) return m;
					k(j, [m])
				} else g && k(j, [mqq.ERROR_NO_SUCH_METHOD]);
			else if (h = "jsbridge://" + encodeURIComponent(c) + "/" + encodeURIComponent(d), l.forEach(function (b, c) {
					mqq.isObject(b) && (b = JSON.stringify(b)), h += 0 === c ? "?p=" : "&p" + c + "=", h += encodeURIComponent(String(b))
				}), "pbReport" !== d && (h += "#" + j), m = p(h, c, d), mqq.iOS && m !== argb && m.result !== argb) {
		...
		function s(b, c, d, e) {
			var f = P[b + "." + c];
			return mqq.isFunction(f) ? f.apply(this, E.call(arguments, 2)) : r.apply(this, E.call(arguments))
		}

		function t(b, c, d, e, g) {
			if (!b || !c || !d) return null;
			var i, j, k = E.call(arguments);
			mqq.isFunction(k[k.length - 1]) ? (g = k[k.length - 1], k.pop()) : g = null, e = 4 === k.length ? k[k.length - 1] : {}, g && (e.callback_type = "javascript", i = h(g), e.callback_name = i), e.src_type = e.src_type || "web", e.version || (e.version = 1), j = b + "://" + encodeURIComponent(c) + "/" + encodeURIComponent(d) + "?" + v(e), p(j, c, d), f(b, c, d, k, i)
		}
		...
		function A(b, c, d) {
			...
			mqq.android && g.options.broadcast === !1 && mqq.compare("5.2") <= 0 && (g.options.domains = ["localhost"], g.options.broadcast = !0), "browser" !== mqq.platform && (e = "jsbridge://event/dispatchEvent?p=" + encodeURIComponent(JSON.stringify(g) || ""), p(e, "event", "dispatchEvent"), f("jsbridge", "event", "dispatchEvent"))
		}
		...
		return D ? (mqq.debuging = !0, C = D.ua || C) : mqq.debuging = !1, mqq.iOS = G.test(C), mqq.android = H.test(C), mqq.iOS && mqq.android && (mqq.iOS = !1), mqq.version = "20160418001", mqq.QQVersion = "0", mqq.clientVersion = "0", mqq.ERROR_NO_SUCH_METHOD = "no such method", mqq.ERROR_PERMISSION_DENIED = "permission denied", mqq.compare = function (b) {
			return c(mqq.clientVersion, b)
		}, mqq.platform = function () {
			var d, e = "browser";
			return mqq.android && ((d = C.match(K)) && d.length ? (mqq.QQVersion = mqq.clientVersion = (c(d[1], d[3]) >= 0 ? d[1] : d[3]) || "0", e = "AndroidQQ") : (d = C.match(L)) && d.length && (mqq.clientVersion = d[1] || "0", e = "AndroidTribe"), window.JsBridge = window.JsBridge || {}, window.JsBridge.callMethod = r, window.JsBridge.callback = l, window.JsBridge.compareVersion = mqq.compare), mqq.iOS && ((d = C.match(J)) && d.length ? (mqq.clientVersion = d[1] || "0", mqq.QQVersion = d[2] || mqq.clientVersion, e = "iPadQQ") : (d = C.match(I)) && d.length ? (mqq.QQVersion = mqq.clientVersion = d[1] || "0", e = "iPhoneQQ") : (d = C.match(L)) && d.length && (mqq.clientVersion = d[1] || "0", e = "iOSTribe"), window.iOSQQApi = mqq, mqq.__RETURN_VALUE = argb), e
		}(), Q = function () {
			var a, b = 1;
			...
			function b() {
				var c, g = d,
					k = {};
				d = [], f = 0, g.length && (k.appid = h, k.typeid = i, k.releaseversion = l, k.sdkversion = mqq.version, k.qua = m, k.frequency = j, k.t = Date.now(), k.key = ["commandid", "resultcode", "tmcost"].join(","), g.forEach(function (a, b) {
					k[b + 1 + "_1"] = a[0], k[b + 1 + "_2"] = a[1], k[b + 1 + "_3"] = a[2]
				}), k = new String(v(k)), mqq.compare("4.6") >= 0 ? setTimeout(function () {
					...
					c = null
				}, c.src = "//wspeed.qq.com/w.cgi?" + k), f = setTimeout(b, e))
			}

			function c(c, h, i) {
				if (mqq.__needReport !== !1) {
					var k;
			...
			var d = [],
				...
				i = 1000280,
				j = 100,
				k = String(mqq.QQVersion).split(".").slice(0, 3).join("."),
				l = mqq.platform + "_MQQ_" + k,
				m = mqq.platform + mqq.QQVersion + "/" + mqq.version;
			return {
				send: c
			}
		}(), e(mqq, function () {
		...
		}()), mqq.__aCallbacks = M, mqq.__aReports = N, mqq.__aSupports = O, mqq.__aFunctions = P, mqq.__fireCallback = k, mqq.__reportAPI = f, e(mqq, {
			invoke             : s,
			invokeClient       : r,
			invokeSchema       : t,
			build              : n,
		...
		iOS    : function (a) {
			...
			a = mqq.extend(d, a || {}, !0), a.callback = mqq.callback(function (c) {
				var d = {
					...
					f = c.code;
				-1 == f && (f = 2), e = e[f], mqq.__execCallback(b, e, d, a)
